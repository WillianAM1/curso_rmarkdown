...
Originalmente a linguagem markdown foi feita para facilitar a geração de arquivos html, então por padrão, R Markdown tem suporte para a criação de arquivos html.
...
Outra opção disponível para o cabeçalho é `toc_float`, quando definida como verdadeira, o sumário não ficara fixo no início do arquivo html, ele ficará sempre visivel, independente da parte do arquivo que esteja sendo visualizada. A opção `toc_float` pode ser definida da seguinte forma:
...
Dentro de `toc_float` existem duas outras opções a serem definidas, `collapsed`, que por padrão é definida como verdadeira, e `smooth_control`, que também é definida como verdadeira por padrão.
